...
public class Myswaggerconfig {
    ...
    public Docket postsApi() {
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(metadata()).select().paths(regex("/Controller.*")).build();
    }

    private ApiInfo metadata() {
        return new ApiInfoBuilder().title("Rick & Morty API")
                .description("API")
                .termsOfServiceUrl("com.example.api")                
                .version("1.0").build();
    }
}
